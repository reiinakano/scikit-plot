...
def plot_validation_curve(clf, X, y, param_name=None, param_range=None,
                          title='validation curve',cv=None,
                          n_splits=5, test_size=.2, random_state=None,n_jobs=1,
                          scoring=None, ax=None, figsize=None, title_fontsize="large",
                          text_fontsize="medium", lw=2):
    """Generates a plot of data validation curves for a classifier of appointed parameter

    Args:
        clf: Classifier instance impliments ``fit`` and ``predict`` method.
        ...
        param_name (string, must):  Parameter name of the classifier needed to be validated.

        param_range (array-like or list-like, must):  Parameter list to be validated, must be 1D
        ...
        n_splits (int, optional): If the cv is not provived, determines how many splits of the data
            to be splited, used in the ShuffleSplit.

        test_size (double, optional): If the cv is not provived, determines what the ratio of the data
            to be test data for cross validation.

        random_state (int, optional): Pseudo-random number generator
            state used for random sampling.
        ...
        text_fontsize (string or int, optional): Matplotlib-style fontsizes.
            Use e.g. "small", "medium", "large" or integer-values. Defaults to
            "medium".
        lw(int, optinal): Matplotlib-style lw.
    ...
    Example:
        ...
        >>> clf = SVC()
        >>> skplt.estimators.plot_validation_curve(clf, X, y, param_name='C', param_range=[.1,10,100])
        <matplotlib.axes._subplots.AxesSubplot object at 0x7fe967d64490>
        >>> plt.show()

        .. image:: _static/examples/plot_validation_curve.png
           :align: center
    ...
    if(param_name is None):
        return 'must give a param to be choosen.'
    if not hasattr(clf,param_name):
        raise TypeError('%s parameter not in classifier.'.format(param_name))
    if(cv is None):
        cv = ShuffleSplit(n_splits= n_splits, test_size=test_size, random_state=random_state)

    ax.set_title(title, fontsize=title_fontsize)
    ax.set_xlabel(param_range, fontsize=text_fontsize)
    ax.set_ylabel('score', fontsize=text_fontsize)
    ax.set_ylim(.0, 1.1)
    ...
    test_scores_mean = np.mean(test_scores,axis=1)
    test_scores_std = np.std(test_scores,axis=1)
    best_param_index = test_scores_mean.argmax()
    best_param = param_range[best_param_index]
    ax.grid()
    # ax.semilogx(param_range, train_scores_mean, label='Training score',
    #             color='darkorange', lw=lw)
    # ax.semilogx(param_range, test_scores_mean, label='Validation score',
    #             color='navy', lw=lw)
    plt.axvline(best_param, linestyle=':', label='Best parameter')
    ax.plot(param_range, train_scores_mean, label='Training score',
            color='r', lw=lw)
    ax.plot(param_range, test_scores_mean, label='Validation score',
            color='g', lw=lw)
    ax.fill_between(param_range, train_scores_mean-train_scores_std,
                    train_scores_mean+train_scores_std, color='r', lw=lw)
    ax.fill_between(param_range, test_scores_mean-test_scores_std,
                    test_scores_mean+test_scores_std, color='g', lw=lw)
    ax.tick_params(labelsize=text_fontsize)
    ax.legend(loc='best', fontsize=text_fontsize)
